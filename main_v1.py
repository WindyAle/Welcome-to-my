...
while running:
    ...
    if is_evaluating:
        ...
        if evaluation_result:
            ...
            wishlist_y_start = popup_rect.bottom - 130

            # 구분선
            pygame.draw.line(screen, (200,200,200),
                             (inner_x, wishlist_y_start),
                             (popup_rect.right - 40, wishlist_y_start), 1)

            # 타이틀
            wishlist_title = font_M.render("요구 가구 목록:", True, (100, 100, 100))
            screen.blit(wishlist_title, (inner_x, wishlist_y_start + 10))
            
            # 가구 목록
            wishlist_str = ", ".join(internal_wishlist)
            draw_text_multiline(
                screen,
                wishlist_str,
                (inner_x, wishlist_y_start + 40),
                font_M, # 일반 폰트
                inner_width,
                (30, 30, 30)
            )
